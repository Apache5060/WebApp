import streamlit as st
import pandas as pd
import numpy as np
from sklearn.linear_model import LinearRegression
import seaborn as sns
import matplotlib.pyplot as plt

header_footer_css = """
    <style>
        .banner {
            position: relative;
            width: 90%;
            height: 100px;  /* Adjust based on your preference */
            background-image: url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQZJkPpcjFpNhfvdUcHdSytP1-tePz8v8X34Q&s');
            background-size: cover;
            color: transparent;  /* Text is transparent by default */
            text-align: center;
            line-height: 100px;  /* Align text vertically */
            transition: color 0.5s ease;  /* Smooth transition for text color */
        }
        .banner:hover {
            color: white;  /* Text color on hover */
        }
    </style>
    """
st.markdown(header_footer_css, unsafe_allow_html=True)
st.markdown("<div class='banner'></div>", unsafe_allow_html=True)
st.markdown("""
        <style>
        .logo {
            position: fixed;
            top: 0;
            right: 0;
            margin-right: 10px;
            margin-top: 10px;
            width: 150px;  /* Adjust this value as needed */
            height: 150px;
        }
        </style>
        """, unsafe_allow_html=True)
st.markdown("""
        <img src="data:image/png;base64," class="logo">
        """, unsafe_allow_html=True)
# Main Streamlit application
st.subheader("Trend Analysis and Data Insights from Historical Financial Statements")

# Add an image to the home page
st.image("https://img.freepik.com/premium-vector/business-statistics-financial-analytics-market-trend-analysis-vector-concept-illustration_92926-2486.jpg", caption="Trend Analysis and Data Insights", use_column_width=True)

# Add an image to the sidebar
st.sidebar.image("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRViRfutvNG9i9GtCPAC6qiwcK_uIOvKU0QP-zvFl3iMHKpUvAvStpetXH8o2AQ_fA4tBg&usqp=CAU", use_column_width=False)

def main():
    uploaded_file = st.sidebar.file_uploader("Choose a CSV or Excel file", type=['csv', 'xlsx'])
    
    if uploaded_file is not None:
        handle_file_upload(uploaded_file)

def handle_file_upload(uploaded_file):
    try:
        if uploaded_file.type == "text/csv":
            df = pd.read_csv(uploaded_file)
            display_file_content(df)
        else:
            handle_excel_file(uploaded_file)
    except Exception as e:
        st.error(f"Error reading file: {e}")

def handle_excel_file(uploaded_file):
    xls = pd.ExcelFile(uploaded_file)
    sheet_names = xls.sheet_names
    
    if 'BS' in sheet_names:
        df = pd.read_excel(uploaded_file, sheet_name='BS')
        display_specific_row(df, "Total non-current assets")
    else:
        sheet = st.selectbox("Choose a sheet", sheet_names)
        df = pd.read_excel(uploaded_file, sheet_name=sheet)
        display_file_content(df, sheet)

def display_file_content(df, sheet=None):
    if sheet:
        st.write(f"Content of the sheet: {sheet}")
    else:
        st.write("File content:")
    st.write(df)

def display_specific_row(df, search_term):
    specific_row = df[df.apply(lambda row: row.astype(str).str.contains(search_term, case=False).any(), axis=1)]
    if not specific_row.empty:
        st.write(f"Row containing '{search_term}':")
        st.write(specific_row)
        forecast_data(specific_row)

def forecast_data(specific_row):
    # Extract the data
    quarters = np.array([1, 2, 3, 4])  # Including the forecast quarter
    values = specific_row.iloc[0][['Q2 22-23', 'Q2 23-24', 'Q3 23-24']].values.astype(float)
    
    # Fit the model
    model = LinearRegression().fit(quarters[:3].reshape(-1, 1), values.reshape(-1, 1))
    
    # Forecast for the next quarter
    forecast_quarter = model.predict(np.array([[4]]))[0][0]
    values = np.append(values, forecast_quarter)  # Append forecast to values

    # Create DataFrame for plotting
    plot_data = pd.DataFrame({
        'Quarter': ['Q2 22-23', 'Q2 23-24', 'Q3 23-24', 'Q4 23-24'],
        'Value': values,
    })

    # Plotting with Seaborn
    sns.set(style="whitegrid")
    plt.figure(figsize=(10, 6))
    # Plot the actual and forecast as one line
    ax = sns.lineplot(x='Quarter', y='Value', data=plot_data, marker='o', color='blue')
    # Highlight the forecast point
    plt.scatter(['Q4 23-24'], [forecast_quarter], color='red', s=100, zorder=5, label='Forecast')
    plt.legend()
    ax.set_title("Forecast of Total Non-Current Assets")
    ax.set_xlabel("Quarters")
    ax.set_ylabel("Value")
    
    st.pyplot(ax.figure)


if __name__ == "__main__":
    main()